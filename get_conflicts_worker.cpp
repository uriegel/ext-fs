...
void Get_conflicts_worker::OnOK() {
    ...
    for (auto item: conflicts) {
        ...
        if (item.source_version.has_value()) {
            auto versionObj = Object::New(Env());
            versionObj.Set("major", Number::New(Env(), static_cast<double>(item.source_version.value().major)));
            versionObj.Set("minor", Number::New(Env(), static_cast<double>(item.source_version.value().minor)));
            versionObj.Set("build", Number::New(Env(), static_cast<double>(item.source_version.value().build)));
            versionObj.Set("patch", Number::New(Env(), static_cast<double>(item.source_version.value().patch)));
        ...
        if (item.target_version.has_value()) {
            auto versionObj = Object::New(Env());
            versionObj.Set("major", Number::New(Env(), static_cast<double>(item.target_version.value().major)));
            versionObj.Set("minor", Number::New(Env(), static_cast<double>(item.target_version.value().minor)));
            versionObj.Set("build", Number::New(Env(), static_cast<double>(item.target_version.value().build)));
            versionObj.Set("patch", Number::New(Env(), static_cast<double>(item.target_version.value().patch)));
